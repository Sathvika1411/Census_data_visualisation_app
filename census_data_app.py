...
if 'Pie Chart' in plot_list :
  ...
  pie_cols = st.sidebar.multiselect("Select the column for pie chart",
                                  ('income', 'gender'))
  for i in pie_cols:
      pie_data = census_df[i].value_counts()
      plt.figure(figsize=(5,5))
      plt.pie(pie_data, labels=pie_data.index, autopct='%1.2f%%', startangle=30)
      plt.title(f'Distribution of records for the {i} feature.')
      st.pyplot()
...
if 'Box Plot' in plot_list :
  ...
  box_cols = st.sidebar.multiselect("Select the column for grouping the distribution of records in the boxplot",
                                  ('income', 'gender'))
  for i in box_cols:
      plt.figure(figsize=(12,2))
      plt.title(f'Box plot for HOurs worked per week for different {i} groups.')
